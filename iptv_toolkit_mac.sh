...
remove_provider() {
    ...
    [[ "$(_lc "$_yn")" == "y" ]] || { echo "Aborted — nothing changed."; return 0; }
...
remove_channel() {
    ...
    _run_config_fn "$config"
    cm_has "$channel" || die "Channel '$channel' not found in '$config'. Available: $(cm_keys | sort | tr '\n' ' ')"
    local id; id="$(cm_get "$channel")"
